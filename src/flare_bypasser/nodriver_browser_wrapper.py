...
In nodriver we use _reliable_call_driver for calls as workaround for problem:
sometimes, chrome don't reply on CDP call and driver hangs on reply waiting.
"""


class NoDriverBrowserWrapper(BrowserWrapper):
  _nodriver_driver: nodriver.Browser = None
  _stopped_process: asyncio.subprocess.Process = None
  _page: nodriver.Tab = None
  ...
  class FakeElement(nodriver.Element):
    ...
    def __init__(self, page: nodriver.Tab, center_coords):
      super(NoDriverBrowserWrapper.FakeElement, self).__init__(
        NoDriverBrowserWrapper.FakeNode(),  # zendriver.cdp.dom.Node
        page  # zendriver.Tab
      )
      self._position = NoDriverBrowserWrapper.FakePosition(center_coords)
  ...
  def __init__(
    self, nodriver_driver: nodriver.Browser, user_data_dir: str = None,
  ...
  async def get_outputs(self):
    try:
      if self._nodriver_driver:  # < driver isn't stopped
        stdout_bytes, stderr_bytes = await self._nodriver_driver.communicate()
  ...
  async def close(self):
    ...
    if self._nodriver_driver:
      self._stopped_process = await self._nodriver_driver.stop()
      self._nodriver_driver = None
    if self._user_data_dir:
      shutil.rmtree(self._user_data_dir, ignore_errors=True)
    ...
    try:
      res = await asyncio.wait_for(
        self._reliable_call_driver(
          nodriver.Tab.select,  # < self._page.select("title", timeout=0)
          self._page,
          "title",
          timeout=0,
      ...
      if "time ran out while waiting for " in str(e).lower():
        # < nodriver timeout on element waiting
        return (None, True)
    ...
    try:
      return len(
        await asyncio.wait_for(
          self._reliable_call_driver(
            nodriver.Tab.select_all,  # < self._page.select_all(css_selector, timeout=0)
            self._page,
            css_selector,
            timeout=0,
  ...
  async def get(self, url):
    ...
    self._page = await self._reliable_call_driver(
      ...
      self._nodriver_driver,  # < self._nodriver_driver.get(url)
      url,
      call_name=("get(" + url + ")"),
      timeout_step=5
    )
  ...
  async def click_coords(self, coords):
    ...
    fake_node = NoDriverBrowserWrapper.FakeElement(self._page, coords)
    await self._reliable_call_driver(
      NoDriverBrowserWrapper.FakeElement.mouse_click,  # < fake_node.mouse_click()
      fake_node,
  ...
  async def get_user_agent(self):
    return await self._reliable_call_driver(
      nodriver.Tab.evaluate,  # < self._page.evaluate("window.navigator.userAgent")
      self._page,
      "navigator.userAgent",
  ...
  async def get_dom(self):
    res_dom = await self._reliable_call_driver(
      nodriver.Tab.get_content,  # < self._page.get_content()
      self._page,
      call_name='get DOM'
    )
    ...
    try:
      while True:
        try:
          ...
          await self._reliable_call_driver(
            nodriver.Tab.save_screenshot,  # < self._page.save_screenshot(tmp_file_path)
            self._page,
            tmp_file_path,
  ...
  async def save_screenshot(self, image_path):
    while True:
      try:
        await self._reliable_call_driver(
          nodriver.Tab.save_screenshot,  # < self._page.save_screenshot(image_path)
          self._page,
          image_path,
  ...
  async def set_cookies(self, cookies: list[dict]):
    ...
    set_cookies = []
    for c in cookies:
      add_cookie = nodriver.CookieParam(
        ...
        expires=(float(c['expires']) if 'expires' in c and c['expires'] is not None else None),  #< push only float to nodriver.
        same_site=c.get('same_site', None)
      )
      set_cookies.append(add_cookie)
    await self._reliable_call_driver(
      getattr(self._nodriver_driver.cookies.__class__, 'set_all'),
      # < self._nodriver_driver.cookies.set_all(set_cookies)
      self._nodriver_driver.cookies,
      set_cookies,
  ...
  async def get_cookies(self) -> list[dict]:
    ...
    nodriver_cookies = await self._reliable_call_driver(
      getattr(self._nodriver_driver.cookies.__class__, 'get_all'),
      # < self._nodriver_driver.cookies.get_all(requests_cookie_format=True)
      self._nodriver_driver.cookies,
  ...
  async def _reliable_call_driver(
    ...
    if self._debug_execution_time:
      ...
      logger.debug(
        "to call '" + (call_name if call_name else NoDriverBrowserWrapper._parse_call(task_fun)) +
        "'"
      )
    finished = False
    try:
      if self._enable_lost_cdp_workaround:
        ...
        res = await NoDriverBrowserWrapper._wait_first([
          NoDriverBrowserWrapper._call_nodriver_async(task_fun, *args, fork_i=0, call_name=call_name, **kwargs),
          NoDriverBrowserWrapper._deffered_call(
            lambda: NoDriverBrowserWrapper._call_nodriver_async(task_fun, *args, fork_i=1, call_name=call_name, **kwargs),
            timeout_step
          ),
          NoDriverBrowserWrapper._deffered_call(
            lambda: NoDriverBrowserWrapper._call_nodriver_async(task_fun, *args, fork_i=2, call_name=call_name, **kwargs),
            2 * timeout_step
          )
        ])
      else:
      ...
      if self._debug_execution_time:
        logger.debug(
          "'" + (call_name if call_name else NoDriverBrowserWrapper._parse_call(task_fun)) +
  ...
  @staticmethod
  async def _call_nodriver_async(
    fun: typing.Callable[typing.Any, typing.Awaitable], *args, fork_i=0, call_name=None,
    **kwargs
  ):
    try:
      logger.debug(
        "call '" + (call_name if call_name else NoDriverBrowserWrapper._parse_call(fun)) +
      ...
      for i in range(max_tries):
        try:
          ...
          if "target must be set to" in str(e) and i != max_tries - 1:
            ...
            # it can appears in nodriver.connection.update_target on all operations,
