...
class ZenDriverBrowserWrapper(BrowserWrapper):
  ...
  class FakeElement(zendriver.Element):
    ...
    def __init__(self, page: zendriver.Tab, center_coords):
      super(ZenDriverBrowserWrapper.FakeElement, self).__init__(
        ZenDriverBrowserWrapper.FakeNode(),  # zendriver.cdp.dom.Node
        page  # zendriver.Tab
      )
      self._position = ZenDriverBrowserWrapper.FakePosition(center_coords)
  ...
  async def create(proxy: bool = None, disable_gpu: bool = False, headless: bool = False):
    ...
    ZenDriverBrowserWrapper.start_xvfb_display()
    ...
    try:
      ...
      return ZenDriverBrowserWrapper(zendriver_driver, user_data_dir=user_data_dir)
  ...
  async def click_coords(self, coords):
    ...
    fake_node = ZenDriverBrowserWrapper.FakeElement(self._page, coords)
    await self._reliable_call_driver(
      ZenDriverBrowserWrapper.FakeElement.mouse_click,  # < fake_node.mouse_click()
      fake_node,
  ...
  async def set_cookies(self, cookies: list[dict]):
    ...
    for c in cookies:
      add_cookie = zendriver.CookieParam(
        ...
        expires=(float(c['expires']) if 'expires' in c and c['expires'] is not None else None),  #< push only float to zendriver.
  ...
  async def _reliable_call_driver(
    ...
    if self._debug_execution_time:
      ...
      logger.debug(
        "to call '" + (call_name if call_name else ZenDriverBrowserWrapper._parse_call(task_fun)) +
        "'"
      )
    finished = False
    try:
      if self._enable_lost_cdp_workaround:
        ...
        res = await ZenDriverBrowserWrapper._wait_first([
          ZenDriverBrowserWrapper._call_zendriver_async(task_fun, *args, fork_i=0, call_name=call_name, **kwargs),
          ZenDriverBrowserWrapper._deffered_call(
            lambda: ZenDriverBrowserWrapper._call_zendriver_async(task_fun, *args, fork_i=1, call_name=call_name, **kwargs),
            timeout_step
          ),
          ZenDriverBrowserWrapper._deffered_call(
            lambda: ZenDriverBrowserWrapper._call_zendriver_async(task_fun, *args, fork_i=2, call_name=call_name, **kwargs),
            2 * timeout_step
          )
        ])
      else:
      ...
      if self._debug_execution_time:
        logger.debug(
          "'" + (call_name if call_name else ZenDriverBrowserWrapper._parse_call(task_fun)) +
  ...
  async def _call_zendriver_async(
    ...
    **kwargs
    ...
    try:
      logger.debug(
        "call '" + (call_name if call_name else ZenDriverBrowserWrapper._parse_call(fun)) +
